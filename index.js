...
if (!fs.existsSync(path.join(cwd, 'working'))) {
    fs.mkdirSync(path.join(cwd, 'working'));
}
...
switch (action) {
    case "Edit Servers":
        ...
        if (fs.existsSync(path.join(cwd, './working/servers.dat'))) {
            //@ts-expect-error
            (_b = (_a = (await nbt.parse(fs.readFileSync(path.join(cwd, './working/servers.dat')), 'big')).parsed.value.servers) === null || _a === void 0 ? void 0 : _a.value) === null || _b === void 0 ? void 0 : _b.value.forEach(server => {
                servers.push({ ip: server.ip.value, name: server.name.value, });
            });
        }
        let serverListFormmatted = [];
        ...
        fs.writeFileSync(path.join(cwd, 'working/servers.dat'), 
        //@ts-expect-error
